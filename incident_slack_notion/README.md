...
- Notion 페이지 생성 시 `장애 보고서` 본문 블록 자동 작성
- 제목만 있고 본문이 비어 있는 기존 Notion 페이지 자동 보정
- 신규 장애 보고서 생성 완료 후 Slack 알림 채널에 Notion 링크 전송
...
- `SLACK_BOT_TOKEN`
- `NOTION_TOKEN`
- `GEMINI_API_KEY`: 선택값. 설정하면 장애 보고서 상단에 3~5줄 LLM 요약을 추가합니다.
...
- `GEMINI_MODEL`: 선택값. 기본값은 `gemini-2.0-flash`

`SLACK_NOTIFICATION_CHANNEL`은 채널명보다 `C...` 또는 `G...` 형식의 채널 ID 사용을
권장합니다. 채널 URL의 `/archives/` 다음 값이 채널 ID입니다. Slack App을 알림 채널에
초대하고 `chat:write` 권한을 부여해야 합니다. 비공개 채널을 이름으로 찾으려면
`groups:read` 권한도 필요합니다.

설정 후 Actions 화면에서 **Slack to Notion incident sync → Run workflow**를 실행합니다.
`Validate integration settings` 단계가 실패하면 해당 단계의 오류에 표시된 Secret 또는
Variable이 누락된 것입니다. Secret 값은 등록 후 GitHub 화면에서 다시 조회할 수 없으며,
이름만 정확히 표시되면 정상입니다.

장애 수집은 15분마다 실행되고, 장애 없음 정상 알림은 한국 시간 기준 `00:17`,
`06:17`, `12:17`, `18:17`에만 전송됩니다. 워크플로 파일이 기본 브랜치에 있어야 예약
실행됩니다. GitHub Actions 예약 실행은 정확한 시각을 보장하지 않으며 부하에 따라
지연될 수 있습니다. 또한 캐시가 삭제되면 SQLite 매핑이 초기화될 수 있으므로, 엄격한
무중단 운영과 1분 주기가 필요하면 GitHub Actions가 아닌 상시 실행 서버에서
`python -m incident_slack_notion.app`을 실행해야 합니다.

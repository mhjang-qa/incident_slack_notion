...
최신 Notion API에서는 Database 아래에 하나 이상의 Data Source가 존재합니다.
애플리케이션은 Database를 조회한 뒤 첫 번째 Data Source를 자동 선택하여 컬럼 스키마
조회와 페이지 생성에 사용합니다.

## 3. 페이지 본문 템플릿

Notion API는 UI에서처럼 Database 템플릿을 선택해 적용하는 흐름을 안정적으로 제공하지
않습니다. 따라서 애플리케이션은 `장애 보고서` 템플릿과 동일한 구조의 본문 블록을
API로 직접 생성합니다.

신규 장애는 제목/속성 생성과 동시에 본문이 작성됩니다. 이미 생성된 페이지가 제목만
있고 본문 블록이 비어 있으면, 다음 동기화 실행 때 동일한 보고서 본문을 자동으로
보정합니다.
